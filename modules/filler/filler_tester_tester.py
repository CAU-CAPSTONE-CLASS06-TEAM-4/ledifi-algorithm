...
def scale_and_detect(PATH, detect, make_txt, make_wav):
    ...
    if detect:
        ...
        nonsilents = detect_nonsilent(lecture_pydub, min_silence_len=60, silence_thresh=-35)
...
def predicting(nonsilents, FILE_PATH, MODEL_PATH, make_result_bool):
    ...
    for nonsilent in nonsilents:
        ...
        if make_result_bool: 
            txtfile.append(lst_to_HMS(nonsilent, 1000) + " " + str(result[0][1] < 0.05))
    ...
    if make_result_bool:
        ...
        for i in range(len(txtfile)):
            f.write(txtfile[i] + " [" + str(predict_lst[i][0]) + ", " + str(predict_lst[i][1]) + "]\n")
        f.close()
